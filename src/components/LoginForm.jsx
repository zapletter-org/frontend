...
export default function LoginForm({ onSubmit }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      // Allow parent to provide real submit handler
      if (onSubmit) {
        await onSubmit(form);
      } else {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 w-full">
      ...
      <div className="space-y-1">
        ...
        <div className="relative group">
          <input
            ...
            value={form.password}
            onChange={handleChange}
            className="w-full rounded-[10px] bg-[var(--z-muted)]/60 border border-white/10 pl-3 pr-10 py-2 text-sm focus-ring placeholder:text-white/40"
            placeholder="••••••••"
          ...
          <button
            ...
            className="absolute inset-y-0 right-0 my-[3px] mr-[3px] grid place-items-center rounded-[8px] px-2 text-white/60 hover:text-white transition-colors group-focus-within:text-[#ff4500]"
          >
      ...
      <div className="flex items-center justify-between text-sm">
        <a href="#" className="underline text-white/80 hover:text-white">Forgot password?</a>
